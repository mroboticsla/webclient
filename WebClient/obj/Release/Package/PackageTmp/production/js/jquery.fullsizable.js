...
(function() {
  ...
  showImage = function(image, direction, shouldHideChrome) {
    ...
    var imageData = images[current_image];

    $(image_holder_id).hide();

    $(image_holder_id).html('<img src="' + image.buffer_src + '" alt="" style="display: block; max-width:' + $(window).width() + 'px; max-height:' + $(window).height() + 'px; height:' + $(window).height() + 'px; width:auto;"/>');
  ...
  makeFullsizable = function() {
    ...
    return $(options.selector).each(function() {
      ...
      image = new Image;
      image.buffer_src = $(this).attr('src');
  ...
  $.fn.fullsizable = function(opts) {
    ...
    $(document).bind('fullsizable:open', function(e, target) {
      ...
      for (_i = 0, _len = images.length; _i < _len; _i++) {
        image = images[_i];
        if (image.buffer_src === $(target).attr('src')) {
